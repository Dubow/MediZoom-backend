...
mpesaRouter.post("/initiate-payment", async (req, res) => {
  ...
  try {
    ...
    const doctorData = await query("SELECT id FROM doctor_profile WHERE user_id = ?", [doctorId]);
    ...
    const paymentData = {
      ...
      Amount: amount,
      PartyA: `254${clientPhoneNumber.substring(clientPhoneNumber.length - 9)}`,
      PartyB: process.env.MPESA_SHORTCODE,
      PhoneNumber: `254${clientPhoneNumber.substring(clientPhoneNumber.length - 9)}`,
    ...
    return res.status(500).json({ error: `Payment initiation failed: ${error.message}` });
  }
});
...
mpesaRouter.post("/result", async (req, res) => {
  ...
  if (ResultCode === 0) {
    const { CheckoutRequestID, Amount, MpesaReceiptNumber, TransactionDate } = result.Body.stkCallback.CallbackMetadata.Item.reduce(
      (acc, item) => {
    ...
    try {
      ...
      res.status(500).json({ ResultCode: 1, ResultDesc: `Failed to process payment: ${error.message}` });
    }
  } else {
    ...
    try {
      ...
      res.status(200).json({ ResultCode: 1, ResultDesc: `Payment failed: ${error.message}` });
    }
  }
});
